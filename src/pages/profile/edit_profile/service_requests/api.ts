import { Pagination, ReviewInput, ServiceInquiry, ServiceInquiryFilter } from '../../../../API';
...
export const listReceivedServiceInquiries = async (input: Pagination) => {
  ...
  return response.data?.listReceivedServiceInquiries.results;
};

export const listSentServiceInquiries = async (input: { pagination: Pagination; filters: ServiceInquiryFilter }) => {
...
export const listCategorizedInquiries = async () => {
  const response = await requestAuthGraphqlOperationStrict(customQueries.listCategorizedServiceInquiries, {});
  return response.data?.listCategorizedServiceInquiries;
};
...
export interface ServiceInquiryReviewInput {
  id: string;
  rating: number;
  review: string;
}
...
export const sendServiceInquiryReview = async (input: ReviewInput) => {
  const response = await requestAuthGraphqlOperationStrict(customMutations.sendServiceInquiryReview, { input });
