...
import ServiceRequestContent from './components/service_request_content';
import { DateFormat } from '../../../../components/date_format';
import QuotationsRequestContent from './components/quotations_request_content';
...
export const ServiceRequestDetails = ({ serviceRequestId, isProfessional, reloadList }: Props) => {
  ...
  const listOfServices =
    serviceRequestDetails?.services?.results?.length! > 3
      ? serviceRequestDetails?.services?.results
          ?.slice(0, 3)
          .map((service) => service?.title)
          .join(', ') + ` +${serviceRequestDetails?.services?.results?.length! - 3}`
      : serviceRequestDetails?.services?.results?.map((service) => service?.title).join(', ');
    ...
    <section className="service-request-details">
      ...
      {serviceRequestDetails ? (
        <>
                ...
                <div className="order-details-table">
                  ...
                  <div className="other-details">
                    ...
                    <Record
                      ...
                      value={serviceRequestDetails?.professional?.company_name! || t(NO_NAME)}
                    />
                    <Record
          ...
          {serviceRequestDetails?.type === 'RFQ' ? (
